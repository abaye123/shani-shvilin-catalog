...
### איך לדעת אילו מארחים לחסום

```bash
python tools/extract_hosts.py app.apk           # רשימה קריאה
python tools/extract_hosts.py app.apk --json    # מערך hosts מוכן להדבקה
```

הכלי קורא את ה-APK - קבצי ה-dex, המשאבים, המניפסט וה-assets - ומחלץ ממנו
כתובות. הוא מפריד בין שלוש רמות ודאות:

1. **הגדרת אבטחת הרשת** של האפליקציה, אם יש. זה האות הכי חזק בקובץ: דומיינים
   שהמפתח כתב במפורש.
2. **מארחים שנמצאו בתוך URL** - `https://...`. אלה כמעט תמיד אמיתיים.
3. **שאר השמות המנוקדים** - ודאות נמוכה, כי הם לא בתוך URL.

**מה הוא מפספס:** כתובת שהאפליקציה מרכיבה בזמן ריצה, מושכת משרת או מצפינה.
**מה הוא מגזים:** אנליטיקס, ברירות מחדל של SDK, קישורי תיעוד וסכימות XML גרים
באותו מאגר מחרוזות. שם חבילה של Java נכתב בדיוק כמו hostname הפוך, ולכן
`com.google.android.gms` לא מופיע ו-`maps.googleapis.com` כן.

**תקרא את הרשימה, אל תדביק אותה.**
